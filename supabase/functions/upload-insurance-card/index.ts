...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Uploading insurance card:', { employee_id, insurer_name, product_name, policy_number });
    ...
    // Generate S3 key
    const s3Key = buildKey(employee, 'insurance', file_name);
    ...
    console.log('Insurance card uploaded successfully:', { s3Key, policy });
